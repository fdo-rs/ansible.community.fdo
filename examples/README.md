...
In cases where manufacturing and owner servers belong to different entities,
it is possible to use custom keys and certificates instead of having them
generated by the manufacturing server. The following examples show how to
distribute keys and certificates to FDO servers. The collection also includes
a role for generating all or a subset of keys and certificates if needed.
...
- name: Generate FDO Keys and Certificates
  ...
  gather_facts: true
  tasks:
    # By default, role generates keys and certificates of all FDO types (diun, device-cA, manufacturer, owner),
    # and saves them into a pre-defined location
    - import_role:
...
After that, you need to copy keys and certificates required by rendezvous and owner servers from localhost
